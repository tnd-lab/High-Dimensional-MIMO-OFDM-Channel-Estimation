...
def train_process(generator, critic, train_loader, g_opt, c_opt):
    ...
    for batch_idx, (h_freqs, pilot_matrices) in enumerate(train_loader):
        ...
        skew_loss = l2_loss(
            skew_cal(h_freqs.pow(2).sum(dim=1).flatten(1)),
            skew_cal(h_freqs_hat_2.pow(2).sum(dim=1).flatten(1)),
        )
        ...
        s_epoch_loss += skew_loss.detach().cpu().item()
        nmse_epoch_score += nmse_score

    avg_s_loss = s_epoch_loss / len(train_loader)
    ...
    return avg_s_loss, avg_g_loss, avg_c_loss, avg_nmse
...
def eval_process(generator, critic, val_loader):
    ...
    metrics = {
        "skewness_loss": val_s_loss / num_batches,
...
def train_and_eval(
    ...
    for epoch in range(epochs):
        # Training
        train_s_loss, train_g_loss, train_c_loss, train_nmse = train_process(
            generator, critic, train_loader, g_opt, c_opt
        )
        train_s_losses.append(train_s_loss)
        ...
        val_s_losses.append(val_metrics["skewness_loss"])
    ...
    plot_skewness_losses(
        train_s_losses,
        val_s_losses,
        f"results/losses/{save_dir}/fold_{fold + 1}_skewness_losses.png",
    )
    plot_generator_losses(
        train_g_losses,
        val_g_losses,
...
if __name__ == "__main__":
    save_dir = f"txant_{num_ut_ant}_rxant_{num_bs_ant}_speed_samples_{number_of_samples}_ebno_{ebno_db}"
